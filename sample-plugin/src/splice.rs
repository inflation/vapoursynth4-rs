...
impl Filter for SpliceFilter {
    ...
    fn create(
        ...
        let num_clips = input.num_elements(key!(c"clips")).unwrap();

        if num_clips == 1 {
            output.consume_node(
                key!(c"clip"),
                input.get_video_node(key!(c"clips"), 0).unwrap(),
                AppendMode::Replace,
            )?;
        } else {
            let nodes = (0..num_clips)
                .map(|i| input.get_video_node(key!(c"clips"), i))
    ...
    const NAME: &'static FilterName = FilterName::from_cstr(c"Splice");
